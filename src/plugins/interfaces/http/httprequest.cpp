...
namespace Http
{
    Request::Request(const HeaderMap& standardRequestHeaders,
    ...
    void Request::dispatchRequest()
    {
        if(!m_staticDirectories.isEmpty())
        {
            const QString fullFilePath = QFileInfo( QLatin1String("./") + QLatin1String(m_requestHeaders.value( "REQUEST_URI")) ).canonicalFilePath();
