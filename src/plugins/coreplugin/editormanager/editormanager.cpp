...
{
    if (d->m_editorAreas.size ())
        return d->m_editorAreas.at(0);

    qCCritical(corepluginLog) << "m_editorAreas is NULL";
...
{
    Q_UNUSED(fileName)
    Q_UNUSED(isExternalEditor)
    QList<Id> allEditorIds;
    QStringList allEditorDisplayNames;

    // Built-in
    const EditorManager::EditorFactoryList editors = EditorManager::editorFactories(false);
...
EditorManager::EditorFactoryList EditorManagerPrivate::findFactories(Id editorId, const QString &fileName)
{
    ...
    if (!editorId.isValid()) {
        ...
        factories = EditorManager::editorFactories(false);
    } else {
        // Find by editor id
        if (IEditorFactory *factory = findById<IEditorFactory>(editorId))
...
{
    Q_UNUSED(allFactories)
    Q_UNUSED(firstMatchOnly)
    Q_UNUSED(list)
