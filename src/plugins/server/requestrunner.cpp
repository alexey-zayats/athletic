...
namespace Server
{
    ...
    , m_request(request)
    {
        Q_ASSERT(QMetaObject::checkConnectArgs(SIGNAL(foo(Server::Request*)), responderSlot));
    ...
    void RequestRunner::run()
    {
        ...
        metaObject->method(index).invoke(responder, Qt::DirectConnection, Q_ARG(Server::Request*, m_request));
