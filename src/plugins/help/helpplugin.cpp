...
void HelpPlugin::extensionsInitialized()
{
    ...
    filesToRegister.append(ICore::documentationPath() + QLatin1String("/athletic.qch"));
...
#ifdef MAC_NATIVE_HELPVIEWER
    // default setting
#ifdef MAC_NATIVE_HELPVIEWER_DEFAULT
...
void HelpPlugin::showContextHelp()
{
    ...
    if (!source.isValid()) {
        ...
        viewer->setHtml(tr("<html><head><title>No Documentation</title>"
            ...
            "</center></body></html>")
            .arg(athleticTheme()->color(Theme::TextColorNormal).name())
            .arg(contextHelpId)
            .arg(athleticTheme()->color(Theme::TextColorNormal).name()));
    } else {
        viewer->setFocus();
