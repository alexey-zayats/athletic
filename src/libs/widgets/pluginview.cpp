...
/*!
    ...
    \sa Widgets::PluginDetailsView
    \sa Widgets::PluginErrorView
*/
    ...
    : QWidget(parent)
{
    ...
    m_categoryView->setColumnWidth(Internal::LoadedColumn, 40);
    ...
    m_sortModel->setSortRole(Internal::SortRole);
...
PluginSpec *PluginView::pluginForIndex(const QModelIndex &index) const
{
    ...
    auto item = dynamic_cast<Internal::PluginItem *>(m_model->itemForIndex(sourceIndex));
...
void PluginView::updatePlugins()
{
    ...
    QList<Internal::CollectionItem *> collections;
    foreach (PluginCollection *collection, PluginManager::pluginCollections()) {
        ...
        collections.append(new Internal::CollectionItem(collection->name(), collection->plugins(), this));
    }
    ...
    if (!plugins.isEmpty()) {
        ...
        collections.append(new Internal::CollectionItem(tr("Utilities"), plugins, this));
    }

    Utils::sort(collections, [](Internal::CollectionItem *a, Internal::CollectionItem *b) -> bool
        { return a->m_name < b->m_name; });

    foreach (Internal::CollectionItem *collection, collections)
...
bool PluginView::setPluginsEnabled(const QSet<PluginSpec *> &plugins, bool enable)
{
    ...
    foreach (PluginSpec *spec, affectedPlugins) {
        ...
        item->updateColumn(Internal::LoadedColumn);
        item->parent()->updateColumn(Internal::LoadedColumn);
